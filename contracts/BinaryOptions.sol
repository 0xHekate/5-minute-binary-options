...
/**
 * @title BinaryOptions - 5 minute binary options
 */
contract BinaryOptions is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    //Dia interface needs to be implement so we avoid low level calls
    ...
    uint256 public oracleLatestRoundId; // converted from uint256
    ...
    function _getPriceFromOracle() internal view returns (uint256, uint256) {
        ...
        require(
            uint256(timestamp) <= leastAllowedTimestamp,
            "Oracle update exceeded max timestamp allowance"
        );
        require(
            priceRoundCounter > oracleLatestRoundId,
            "Oracle update roundId must be larger than oracleLatestRoundId"
        );
        return (priceRoundCounter, uint256(price));
    }
