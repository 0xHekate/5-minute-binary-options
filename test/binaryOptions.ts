...
import { DIAOracle } from "../typechain";
const binaryOptions = artifacts.require("BinaryOptions");
const Oracle = artifacts.require("DIAOracle");

// const GAS_PRICE = 1000000007; // hardhat default
...
const MIN_BET_AMOUNT = ether("1"); // 1
...
contract(
  ...
  ([
    ...
    beforeEach(async () => {
      ...
      ); // To update Oracle roundId

      prediction = await binaryOptions.new(
        oracle.address,
        admin,
        operator,
        INTERVAL_SECONDS,
    ...
    it("Should start genesis rounds (round 1, round 2, round 3)", async () => {
      ...
      assert.equal(await prediction.genesisLockOnce(), false);
      assert.equal(
        (await prediction.rounds(1)).startTimestamp.toNumber(),
        currentTimestamp
      );
      assert.equal(
        (await prediction.rounds(1)).lockTimestamp.toNumber(),
        currentTimestamp + INTERVAL_SECONDS
      );
      assert.equal(
        (await prediction.rounds(1)).closeTimestamp.toNumber(),
      ...
      assert.equal(
        (await prediction.rounds(1)).lockPrice.toNumber(),
        INITIAL_PRICE
      );

      // Start round 2
      assert.equal(
        (await prediction.rounds(2)).startTimestamp.toNumber(),
        currentTimestamp
      );
      assert.equal(
        (await prediction.rounds(2)).lockTimestamp.toNumber(),
        currentTimestamp + INTERVAL_SECONDS
      );
      assert.equal(
        (await prediction.rounds(2)).closeTimestamp.toNumber(),
      ...
      // End round 1
      assert.equal(
        (await prediction.rounds(1)).closePrice.toNumber(),
        INITIAL_PRICE
      );

      // Lock round 2
      assert.equal(
        (await prediction.rounds(2)).lockPrice.toNumber(),
        INITIAL_PRICE
      );
    });
